...
async function testCompanyEnrichment() {
  ...
  try {
    ...
    testData.forEach((item, index) => {
      console.log(
        `   ${index + 1}. ${item.nome} - ${item.empresa} (CNPJ: ${item.cnpj})`,
      );
    });
    ...
    // Gerar ID de sessão
    const sessionId = dataEnrichmentService.generateSessionId(
      testData,
      'company',
    );
    ...
    const onProgress = (progress: any) => {
      console.log(
        `📈 Progresso: ${progress.percentage}% (${progress.processed}/${progress.total}) - Batch ${progress.currentBatch}`,
      );
    };
    ...
    const onPartialResult = (results: any[]) => {
      ...
      results.forEach((result, index) => {
        ...
        console.log(
          `📊 Dados originais: ${result.data.nome} - ${result.data.empresa}`,
        );
        ...
        if (result.enrichedFields.error) {
          ...
          console.log(
            `📍 Endereço: ${enriched.address.street}, ${enriched.address.number}`,
          );
          console.log(
            `🌆 Cidade: ${enriched.address.city}/${enriched.address.state}`,
          );
    ...
    console.log(
      `⏱️  Tempo total: ${
        finalSession?.endTime
          ? Math.round(
              (finalSession.endTime.getTime() -
                finalSession.startTime.getTime()) /
                1000,
            )
          : 'N/A'
      } segundos`,
    ...
    // Estatísticas
    const successCount =
      finalSession?.results.filter((r) => r.enrichedFields.success).length || 0;
    const errorCount =
      finalSession?.results.filter((r) => r.enrichedFields.error).length || 0;
